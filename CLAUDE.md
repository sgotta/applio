...
- `languages[]` — Languages with proficiency level (optional, visibility toggle)
...
- `templateId` — Active CV template (`"classic"` | `"noPhoto"`) — controls layout rendering

`TemplateId = "classic" | "noPhoto"`. Templates affect both the interactive editor (`CVPreview.tsx`), the shared CV view (`PrintableCV.tsx`), and the PDF output (`pdf-document.tsx`).
...
└── components/
    ...
    └── cv-editor/
        ...
        ├── Languages.tsx            # Languages section (drag-and-drop, noPhoto variant)
...
**Template issues (shared CV always shows Classic):**
- `shared-cv-content.tsx` passes `templateId` to `PrintableCV` — if `PrintableCV` doesn't have the template branch, it falls back to Classic silently
