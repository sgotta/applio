...
**Key characteristics:**
- 25+ CRUD methods (add, update, remove, move operations)
...
- Migration functions for backward compatibility (`migratePersonalInfo`, `migrateCVData`)
...
- `personalInfo` - Name, title, photo (base64), contacts array
- `summary` - About me text
- `experience[]` - Work history with bullet points
- `education[]` - Academic background
- `skills[]` - Categorized skills
- `customSections[]` - User-created sections with `placement: "left" | "right"`
...
└── AppContent
    ...
    │       ├── LEFT COLUMN
    │       │   ├── PersonalInfo (photo, contacts, summary, skills)
    │       │   └── CustomSection[] (left placement)
    │       └── RIGHT COLUMN
    │           ├── CVHeader (name & title, editable)
    │           ├── Experience (work history)
    │           ├── Education
    │           └── CustomSection[] (right placement)
...
**Migration strategy:**
- Old format (PersonalInfo with individual fields) → new format (contacts array)
- `migratePersonalInfo()` runs on load if old format detected
...
└── components/
    ...
    └── cv-editor/           # CV editing components
        ...
        ├── PersonalInfo.tsx     # Left column (photo, contacts, skills)
        └── CustomSection.tsx    # Reusable custom section component
```
...
**Migration issues:**
- Check `migratePersonalInfo()` in cv-context.tsx
